        ...
        messageListener = ListenerService.shared.messagesObserve(chat: chat, completion: { (result) in
            ...
            case .success(var message):
                if let url = message.downloadURL {
                    StorageService.shared.dowloadImage(url: url) { [weak self] (downloadResult) in
    ...
    private func sendImage(_ image: UIImage) {
        StorageService.shared.uploadImageMessage(photo: image, to: chat) { (result) in
            ...
            case .success(let url):
                ...
                FirestoreService.shared.sendMessage(chat: self.chat, message: message) { (sendResult) in
                    switch sendResult {
                    case .success():
                        DispatchQueue.main.async {
                            self.messagesCollectionView.scrollToBottom(animated: true)
                    ...
                    case .failure(let error):
                        self.showAlert(with: "Ошибка!", and: "Изображение не было доставлено. Детали \(error.localizedDescription)")
                    }
                }
            case .failure(let error):
                self.showAlert(with: "Ошибка!", and: error.localizedDescription)
...
extension ChatsViewController {
    ...
    private func configureCameraIcon() {
        ...
        cameraItem.addTarget(
            self,
            action: #selector(cameraButtomPressed),
...
extension ChatsViewController: MessagesDisplayDelegate {
    ...
    func backgroundColor(for message: MessageType, at indexPath: IndexPath, in messagesCollectionView: MessagesCollectionView) -> UIColor {
        return isFromCurrentSender(message: message) ? .systemGray5 : #colorLiteral(red: 0.7882352941, green: 0.631372549, blue: 0.9411764706, alpha: 1)
    }
